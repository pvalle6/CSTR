...
def print_simulation_data():
    ...
    :return:
    """
    print(simulation_data.to_string())
    ...
    for i in range(0, 10000):
        ...
        sol = scipy.integrate.solve_ivp(system_of_equations, [last_time, last_time + 0.1],
                                        [history_df['bulk_a_comp'].iloc[-1], history_df['reactor_temp'].iloc[-1],
                                         history_df['jacket_temp_out'].iloc[-1]],
                                        args=(feed_flow, feed_comp_a, feed_temp, jack_feed_flow), method="BDF")
        ...
        simulation_data = pd.concat([simulation_data, pd.DataFrame(
            {"time": last_time, "flow101": flow101_indicator.get(),
...
def open_sensor_graph(sensor_name):
    ...
    sensor_window.geometry("500x500")

    sensor_graph = Figure(figsize=(6, 5), dpi=100)
...
reactor_temp_indicator.set("300")
...
cool_temp_out_indicator.set("295")
